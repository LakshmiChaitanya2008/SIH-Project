 ...
 * Client-Side In-Browser Implementation
 * 
 * Computes exact normalized cosine similarity on 768-dim embeddings,
 * applies geographic and temporal weighting, and clusters civic signals into
 * validated community problem patterns without requiring any serverless functions.
 */
...
export function clusterSubmissions(submissions, options = {}) {
  const envThresh = (typeof import.meta !== 'undefined' && import.meta.env?.VITE_CLUSTER_SIMILARITY_THRESHOLD) ||
                    (typeof process !== 'undefined' && process.env?.CLUSTER_SIMILARITY_THRESHOLD)
  const similarityThreshold = parseFloat(options.similarityThreshold || envThresh || 0.78)
  const minClusterSize = parseInt(options.minClusterSize || 3, 10)
  ...
  for (const sub of submissions) {
    ...
    if (Array.isArray(sub.embedding)) {
      ...
      try {
        emb = typeof sub.problem_embeddings[0].embedding === 'string'
          ? JSON.parse(sub.problem_embeddings[0].embedding)
          : sub.problem_embeddings[0].embedding
      } catch (e) {
        emb = null
      }
